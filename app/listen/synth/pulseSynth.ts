...
export class PulseSynth {
  private synth0         : Tone.PolySynth<Tone.FMSynth>;
  private synth1         : Tone.PolySynth<Tone.FMSynth>;
  ...
  constructor() {
    ...
    // Connect the chain
    this.synth0.chain(this.filter, this.chorus, this.delay, this.gain, Tone.Destination);
    this.synth1.chain(this.filter, this.chorus, this.delay, this.gain, Tone.Destination);
  }
  ...
  playChord(notes: Tone.Unit.Frequency[], duration: Tone.Unit.Time = '2n', time?: Tone.Unit.Time): void {
  ...
  dispose(): void {
    ...
    this.punchEnvelope.dispose();
    this.punchGain.dispose();
  }
}
