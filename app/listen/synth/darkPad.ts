...
export class DarkAmbientPad {
  ...
  constructor() {
    ...
    this.bitCrusher = new Tone.BitCrusher({
        bits : 8
      , wet  : 0.0  // Start at 0, can be increased for grittiness
    });
    ...
    this.layer1Gain = new Tone.Gain(0.200);
    this.layer2Gain = new Tone.Gain(0.150);
  ...
  playChord(notes, duration = '4n', time = undefined, withSub = true): void {
  ...
  playChordEternal(notes, time, withSub = true): void {
  ...
  setFilterCutoff(freq, rampTime = 2): void {
    this.filterLFO.set({ 
      min: freq - 200, 
      max: freq + 400 
    });
