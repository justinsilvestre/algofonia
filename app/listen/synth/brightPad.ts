...
export class BrightAmbientPad {
  ...
  constructor() {
    ...
    this.warmGain     = new Tone.Gain(0.40);
    this.sparkleGain  = new Tone.Gain(0.20);
  ...
  playChord(notes, duration = '4n', time = undefined, withSparkle = true): void {
  ...
  // Eternal drone mode
  playChordEternal(notes, time, withSparkle = true): void {
  ...
  setSparkleRate(rate): void {
    this.brightnessLFO.frequency.value = rate;
  }
  
  setSparkleDepth(min, max): void {
