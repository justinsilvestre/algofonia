...
export class AbstractHummingSampler {
  loaded: boolean;
  bufferDuration: number;
  players: Tone.Player[];
  playerChains: PlayerChain[];
  delay1: Tone.FeedbackDelay;
  delay2: Tone.FeedbackDelay;
  shimmerReverb: Tone.Reverb;
  chorus: Tone.Chorus;
  filter: Tone.Filter;
  freqShift: Tone.FrequencyShifter;
  ...
  constructor(sampleUrl: string) {
    ...
    this.players.forEach((player: Tone.Player, i: number) => {
      ...
      const panner = new Tone.Panner(Math.sin(i) * 0.7);

      // Individual volume control for waves
      const gain = new Tone.Gain(0); // Start silent
  ...
  updateWaves(entropy: number, masterVolume: number): void {
    ...
    this.tremolo.frequency.rampTo(map(entropy, 0, 1, 0.08, 13), 0.1);

    // Evolving panning for spatial movement
    this.playerChains.forEach((chain: PlayerChain, i: number) => {
      const panPhase = this.wavePhase + i * 0.3;
      chain.panner.pan.rampTo(Math.sin(panPhase) * 0.8, 3.0);
    });
    ...
    this.players.forEach((player: Tone.Player, i: number) => {
      ...
      const layerPhase = this.wavePhase + i * 0.7;
      const wave = (Math.sin(layerPhase) + 1) / 2; // 0 to 1
